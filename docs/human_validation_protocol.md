...
| **OF-3** | Upstream GWT dataset **license "not stated"**. | **BLOCKING (external release)** | `data_governance_checklist.md` §1 | ▢ Resolve license before any external distribution. | | |
| **OF-4** | `n_donors` is always NaN. | Low | data_dictionary §2 | ▢ Confirm honest placeholder (data absent), not a broken join. | | |
...
| **OF-7** | Doc-vs-code path drift: `data_dictionary.md` still cites `build_target_cards.py::build_cards_frame` / `readiness_engine.py`; the live code is `core/cards.py` / `core/readiness.py` (`build_target_cards.py` is a back-compat shim). Also minor count drift (sample_metadata 11 vs 12; disease rows 7,528 vs 7,527). | Low | this protocol §1, §2 | ▢ Refresh the dictionary's code-path references + counts. | | |
