...
| `sources/target_tool_cache/_overlays/gtex_per_tissue.parquet` | §1.12 GTEx safety-window overlay snapshot (pre-aggregated per-gene, off-context tissues only) | **Tracked** (intentional real-data snapshot, public-database-derived per `docs/data_governance_checklist.md`) | Static, no TTL — a point-in-time snapshot of public GTEx data, not live-refreshed. Replace manually if a newer snapshot is obtained; `safety_overlay.py`'s loader reads whatever file is present with no version check today (a gap noted, not silently accepted — same caveat as static overlay lists in §3). The membrane/tractability half of §1.12 reads `docs/mvp-research/adc_overlay_gwt_overlap_full.csv` instead (not this directory). |
