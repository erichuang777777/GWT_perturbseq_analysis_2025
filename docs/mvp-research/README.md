...
- 模組三(群體假設引擎)維持**不接受任何個體病人層輸入**——只做 gene-level 群體統計查表或證據匹配。**例外(2026-07 新增,見 `docs/compass_concept_integration_plan.md`):** 獨立的「個體概念剖面」探索 demo 模組接受單一樣本的基因表現向量,但**僅做透明的概念投影**(表現向量 → 20 個 CD4 免疫概念活化剖面),不做黑箱預測、不輸出診斷/治療/療效預測、不儲存或外傳原始輸入、無識別欄位;明確定位為探索性研究 demo,非正式醫療軟體。概念剖面為描述性,不進 `readiness_call`/`_stage()`
