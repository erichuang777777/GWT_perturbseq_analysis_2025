...
chart_id        : D6
source_image    : D6_strip.png
chart_title     : On-target effect size across conditions for 15 prioritized targets
language        : Python
env_name        : figures
...
for i,c in enumerate(COND_ORDER):
    ...
    ax.scatter(x,v,s=34,color=PAL[c],alpha=0.85,edgecolor='white',lw=0.6,zorder=3)
    med=np.median(v); ax.plot([i-0.22,i+0.22],[med,med],color='0.15',lw=2.4,zorder=4)
...
ax.set_title('All 15 prioritized targets maintain strong knockdown across conditions; line = median effect size', fontproperties=CJK, loc='left')
ax.set_xlim(-0.5,2.5); ax.set_ylim(ymin-5, 3.5)
fig.tight_layout(); fig.savefig('D6_strip.png',dpi=300,bbox_inches='tight')