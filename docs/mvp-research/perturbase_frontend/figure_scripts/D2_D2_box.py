...
bp=ax2.boxplot([grp[c]['logDE'].values for c in COND_ORDER],positions=[0,1,2],widths=0.55,patch_artist=True,showfliers=True,
   flierprops=dict(marker='o',ms=2,mfc='0.6',mec='none',alpha=0.22),medianprops=dict(color='black',lw=2),
   whiskerprops=dict(color='0.3',lw=1.1),capprops=dict(color='0.3',lw=1.1))
for i,c in enumerate(COND_ORDER):
    bp['boxes'][i].set_facecolor(PAL[c]); bp['boxes'][i].set_alpha(0.8); bp['boxes'][i].set_edgecolor('0.3'); bp['boxes'][i].set_linewidth(0.9)
style_logde_ax(ax2)
ax2.set_title('Median is 2 DE genes in all conditions; IQR consistent across the three',fontproperties=CJK,loc='left')
fig2.subplots_adjust(left=0.15); fig2.savefig('D2_box.png',dpi=300,bbox_inches='tight')