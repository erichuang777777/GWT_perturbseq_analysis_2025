...
| File | Location committed | Role |
|------|--------------------|------|
| `reproducibility.yml` | `.github/workflows/` | The GitHub Action. |
| `verify_reproducibility.py` | `docs/mvp-research/pipeline/reproducibility_audit/` | Standalone runner — the actual guard logic. Runs locally **and** from CI. |
| `CI_REPRODUCIBILITY_README.md` | next to the workflow, or in the audit dir | This document. |

The bundle itself is committed as a single compressed archive,
`docs/mvp-research/pipeline/reproducibility_audit/reproducibility_bundle_v2.tar.gz`
— the CI workflow extracts it to `reproducibility_bundle/` (uncompressed) at the
start of the job; that extracted directory is `.gitignore`d, not committed
separately, to avoid keeping two copies of the same ~compressed-then-uncompressed
data in the repo. **No new raw data is added** — the raw table is published
bioRxiv-preprint supplementary data, already public in the repo. CI only reads
it via `actions/checkout` + the extraction step.
