...
| `safety_window_score` | `0`, an int 0-30 (GTEx tissue-breadth count), or `"unknown"` | `0` if the gene is a known essential gene (always wins); else the count of GTEx tissues (of 30) where the gene clears a TPM>1 detectable-expression threshold, from `safety_overlay.safety_window_from_gtex()` (§1.12), if the gene is in the ~9,727-gene GTEx overlay; else `"unknown"`. Higher = more broadly expressed across normal tissues (not yet collapsed into a tight/moderate/wide tier -- the raw count is kept visible) | `_stage()`/`readiness_call` never depend on this column -- it is descriptive only |
