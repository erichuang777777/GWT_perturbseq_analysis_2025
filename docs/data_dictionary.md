...
| `metadata/suppl_tables/sgrna_library_metadata.suppl_table.csv` | 26,504 | `target_gene_id` (Ensembl ID), `target_gene_name` (curated canonical symbol), `target_gene_name_from_sgRNA` (sgRNA-design-time symbol) | 12,654 unique targeted genes; 344 have a real design-time-vs-curated symbol difference. Source of B1's alias table. Row count is the parsed-record count (`pandas`), not `wc -l` — some fields contain embedded newlines, so raw line count (31,109) over-counts. |
| `metadata/suppl_tables/sample_metadata.suppl_table.csv` | 12 | `culture_condition`, `10xRun_ID` (or `run_id`) | Used to detect condition/run confounds (`confounded_conditions()`). 12 samples = 4 donors × 3 culture conditions. |
...
| `src/6_functional_interaction/results/disease_gene_associations_detailed.csv` | 7,527 | `disease_efo`, `disease_name`, `gene_symbol`, `association_score`, `genetic_evidence_score`, `genetic_evidence_types` | 13 autoimmune/inflammatory indications; feeds `disease_translator.py`. |
...
| `safety_window_score` | an int 0-30 (off-context GTEx tissue-breadth count) or `"unknown"` | The count of GTEx tissues (of 30, Blood/Spleen excluded -- see §1.12's context-inversion note) where the gene clears the source database's expression threshold, from `safety_overlay.safety_window_from_gtex()`, if the gene is in the ~9,718-gene GTEx overlay; else `"unknown"`. Higher = more broadly expressed outside CD4 context (not yet collapsed into a tight/moderate/wide tier -- the raw count is kept visible). **Not gated on gene essentiality** -- essential genes are already fully captured by the separate `essential_gene` red-flag override and `_stage()`'s grade cap (see `red_flag_override`/`overall_readiness_stage` below); an earlier version of this column zeroed itself for essential genes, which silently discarded real GTEx data and showed the *safest-looking* value (0) for the riskiest gene class -- fixed to report the real overlay value (or honest `"unknown"`) unconditionally, per this repo's `unknown != 0` invariant | `_stage()`/`readiness_call` never depend on this column -- it is descriptive only |
| `gnomad_constraint_flag` | `"loss_intolerant"`/`"none"`/`"unknown"` | Second, independent safety signal (§C of `docs/next_phases_plan.md`): `"loss_intolerant"` if the gene's gnomAD LOEUF < 0.6 (a deliberately inclusive "constrained-ish" soft cutoff over the real v2.1.1 LOEUF distribution — more permissive than gnomAD's own strict bottom-decile ~0.35 line; originally proposed in `docs/mvp-research/ENHANCEMENT_連結器加強建議.md` §2); `"none"` if the gene is in the overlay but doesn't clear that bar; `"unknown"` if the gene is absent from the overlay or the overlay is unavailable -- from `safety_overlay.gnomad_flag_from_constraint()`, when `gnomad_overlay` is supplied. Backed by the full-genome gnomAD **v2.1.1** by-gene snapshot (19,155 genes, chrX included; `sources/target_tool_cache/_overlays/gnomad_constraint_seed.csv`), built reproducibly by `data_acquisition/build_gnomad_constraint_overlay.py`; covers 11,267/11,526 targets (~97.8%) | `_stage()`/`readiness_call` never depend on this column -- it is descriptive only, same causal-independence property as `safety_window_score` |
| `gnomad_loeuf`, `gnomad_pli` | float or `"unknown"` | Raw gnomAD LOEUF / pLI passthrough values for the gene, if present in the gnomAD constraint overlay, else `"unknown"` | `readiness_engine._gnomad_loeuf_pli()`, when `gnomad_overlay` is supplied |
| `genetic_support_confidence` | `"strong_genetic_association"`/`"moderate_genetic_association"`/`"no_genetic_association"`/`"unknown"` | Roadmap P1.1/P1.2 genetic-support tier, graded from the injected Open Targets evidence snapshot (`sources/target_tool_cache/_evidence/<gene>.json`): the strongest `associated_diseases[].genetic_association_score` maps to `strong` (>= 0.5) / `moderate` (0-0.5) / `no_genetic_association` (gene resolved in Open Targets but no positive germline genetic score — e.g. VAV1, whose high overall scores are cancer literature/somatic, not genetics); `"unknown"` when no snapshot was fetched, Open Targets was unavailable, or the gene wasn't resolved (**`unknown` != `no`** — unmeasured is not absence). Graded **on `genetic_association_score` only**, deliberately not `overall_score`. Rationale: genetic support ~2.6x clinical success rising with causal-gene confidence (Minikel *Nature* 2024); presented as GRADED, never authoritative, because L2G/eQTL do not beat nearest-gene (medRxiv 2025). From `common.evidence_grading.genetic_support_confidence_from_evidence()`, when `evidence_lookup` is supplied | `_stage()`/`readiness_call` never depend on this column — it is descriptive only, same causal-independence property as `safety_window_score`/`gnomad_constraint_flag` |
| `genetic_support_max_genetic_score` | float or `"unknown"` | Raw passthrough: the strongest Open Targets `genetic_association_score` across the target's associated diseases (`0.0` when the gene is resolved but no disease carries a positive genetic score; `"unknown"` when unmeasured) — for auditing `genetic_support_confidence` | `common.evidence_grading.genetic_support_confidence_from_evidence()` |
| `composite_safety_liability` | `"high"`/`"moderate"`/`"low"`/`"unknown"` | Roadmap P1.3 composite on-target safety **LIABILITY** tier composing the two real overlays: gnomAD LoF constraint (`gnomad_constraint_flag`) + GTEx off-context expression breadth (`safety_window_score` >= 26, the real median of the ~9,718-gene overlay). `high` = loss-intolerant **and** broadly expressed; `moderate` = exactly one; `low` = neither; `"unknown"` if **either** component is `"unknown"` (availability propagates — an unmeasured component is never treated as "no risk"). **This is a LIABILITY flag, NOT de-risking**: higher constraint + broader expression = MORE concern, never "this target is safe" (genetic support predicts on-target liabilities — Duffy *Sci Adv* 2020; Nat Rev Genet 2025). Both component columns stay independently visible. From `common.overlay_lookup.composite_safety_liability()`, when `gnomad_overlay` + `gtex_overlay` are supplied | `_stage()`/`readiness_call` never depend on this column — it is descriptive only |
| `trait_liability_similarity` | `"unknown"` (honest-fallback) | Roadmap P1.3 on-target trait-similarity liability — would flag a target whose genetically-associated traits resemble known adverse-event terms (a liability, not de-risking). **Honest-fallback stub**: this repo commits no adverse-event reference vocabulary (e.g. SIDER/MedDRA), so it returns `"unknown"` + a reason rather than a fabricated similarity score. A real vocabulary dropped in later enables `match`/`no_match` with no caller change | `common.evidence_grading.trait_liability_similarity()` |
| `trait_liability_similarity_reason` | str | Why `trait_liability_similarity` is `"unknown"` (no committed adverse-event reference vocabulary) | `common.evidence_grading.trait_liability_similarity()` |
