...
| `gnomad_constraint_flag` | `"loss_intolerant"`/`"none"`/`"unknown"` | Second, independent safety signal (§C of `docs/next_phases_plan.md`): `"loss_intolerant"` if the gene's gnomAD LOEUF < 0.6 (gnomAD **v4** "constrained" threshold; broadened from the v2.1.1-era 0.35 originally proposed in `docs/mvp-research/ENHANCEMENT_連結器加強建議.md` §2, now that the seed carries real v4 LOEUF/pLI values); `"none"` if the gene is in the overlay but doesn't clear that bar; `"unknown"` if the gene is absent from the overlay or the overlay is unavailable -- from `safety_overlay.gnomad_flag_from_constraint()`, when `gnomad_overlay` is supplied. Currently backed by a 15-gene v4 seed (`sources/target_tool_cache/_overlays/gnomad_constraint_seed.csv`); a full-genome gnomAD snapshot upgrades coverage with no code change | `_stage()`/`readiness_call` never depend on this column -- it is descriptive only, same causal-independence property as `safety_window_score` |
