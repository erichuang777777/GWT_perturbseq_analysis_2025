...
from core.readiness import compute_readiness, load_overlays, readiness_summary
...
from evidence.disease import list_diseases, load_disease_associations, translate_disease
from resolve.resolver import load_resolver, result_status
from resolve.search import search_genes
from resolve.cre import cre_for_gene, load_cre_elements, load_variant_cre_links
from evidence.population import CAVEAT_TEXT, build_population_hypothesis_card, load_burden_estimates
from evidence.safety_overlay import load_gnomad_constraint_overlay, load_gtex_safety_overlay, load_membrane_tractability_overlay
...
# SRC == src/3_DE_analysis (the package root one level up from this api/ dir,
# not this file's own parent -- target_card_api.py's original ROOT/SRC
# assumed a flat, one-level-deep file; api/app.py is now two levels deep, see
...
# subprocess (see _run_script below), which only auto-adds the *script's
# own* directory to sys.path. core/cards.py's package-relative imports
# (`from core.kd_status import ...`) require src/3_DE_analysis itself (not
# core/) on sys.path, so only the top-level shim -- which sits directly in
# src/3_DE_analysis -- is safely subprocess-invocable this way.
