...
Split out of the original flat ``build_target_cards.py`` (architecture
refactor Phase 2 -- see ``docs/architecture_refactor_plan.md`` §3);
``kd_status``/``statistical_evidence_grade`` scoring moved to the sibling
``core.kd_status``/``core.scoring`` modules, and the (previously duplicated)
gene-list/overlay file loaders moved to ``data.loaders``. This module keeps
the original public names as thin wrappers so existing callers
(``from build_target_cards import build_cards_frame`` etc., via the
``build_target_cards.py`` re-export shim) are unaffected.

This combines DE_stats and guide KD tables into a compact, reproducible
...
from core.kd_status import KD_NOT_MEASURABLE_EXPRESSION_FLOOR, KD_THRESHOLD_VERSION, kd_status as _kd_status
from core.scoring import make_score as _make_score, score_cap_reasons as _score_cap_reasons
...
# Re-exported from data.loaders (the single implementation, architecture
# refactor Phase 2 -- previously duplicated byte-for-byte in
# readiness_engine.py as `load_overlays`).
load_gene_set = _loaders.load_gene_set
...
def load_druggable_overlays(gene_lists_dir: Path) -> Dict[str, set]:
    ...
    return _loaders.load_druggable_class_overlays(gene_lists_dir, DRUGGABLE_CLASS_MODALITY)
...
def _load_benchmark(path: Optional[Path]) -> Optional[pd.DataFrame]:
    return _loaders.load_benchmark_csv(path)
